...
const Signin = () => {
  ...
  const signin = async (values: object) => {
    ...
    if (isSuccessful(response?.status)) {
      ...
      status && navigate("/shipments", { replace: true, state: { isLoggedIn: true } });
    ...
    <Fragment>
      <div className="signin">
        ...
        <div className="auth-layout__subheading fw-medium mst-10 msb-32">
          Log into your account in order to continue enjoying amazing <br />rates on every shipment you make!
        </div>
          ...
          {props => (
            <Form className="w-100 grid place-items-center">
              ...
              <div className="form-group w-100 msb-24">
                <Field name="password" as={FormField.PasswordField} placeholder="Password" required />
              ...
              <div className="auth-layout__text">
                New User?
                <NavLink to="/signup">
                  <span className="auth-layout__link msl-2">Create an account</span>
                </NavLink>
              </div>
            </Form>
          )}
