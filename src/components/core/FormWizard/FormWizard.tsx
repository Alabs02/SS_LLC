...
class FormWizard extends Component {
  static Wrapper = (_props: IChildren) => {
    return <div className="form-wizard">{_props.children}</div>;
  };
  ...
  static Panel = (_props: IFormWizardPanel) => {
    ...
    const renderTabItems = () => {
      return _props.tabs.map((tab: ITabItem, index: number) => (
        <button
          onClick={() => _props.setSelectedTab(tab.tabID)}
          type={'button'}
          key={tab.tabID + index}
          className="form-wizard__item msb-20 msb-md-0"
        >
          <div
            className={classnames(
              'form-wizard__item-circle',
              tabClassnames(tab)
            )}
          >
            <tab.icon
              fontSize={20}
              className={classnames(tabIconClassnames(tab))}
            />
          </div>
          <div
            className={classnames(
              'fs-14 lh-20 mst-4 fw-semi-bold',
              tabTitleClassnames(tab)
            )}
          >
            {tab.title}
          </div>
    ...
    return (
      <Fragment>
        <div className="form-wizard__panel">{renderTabItems()}</div>
      </Fragment>
    );
  };
  ...
  static Content = (_props: IFormWizardContent) => {
    return _props.tabID === _props.selectedTab ? (
      <div className="form-wizard__content mst-60">{_props.children}</div>
    ) : null;
  };
